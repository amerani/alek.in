...
async function publishS3() {
    ...
    console.log(chalk.green(`Successfully cleaned S3 bucket ${process.env.AWS_S3_BUCKET}`));
    ...
    const puts = files.map(file => {
        ...
        return S3.putObject({
            Bucket,
            Key,
            Body: fs.createReadStream(path.resolve(dist, Key))}).promise();
    });
    await Promise.all(puts);

    console.log(chalk.green(`Successfully updated S3 bucket ${process.env.AWS_S3_BUCKET}`));
}
